...
public:
      ...
      SimpleScenes(world)
	{
      ...
      virtual void frameEnded(Real time, OIS::Keyboard* input, OIS::Mouse* mouse)
      {
		...
		// Up
		if (input->isKeyDown(OIS::KC_X)) _force_to_apply += Ogre::Vector3::UNIT_Y * _force;

		// Left/right
		if (input->isKeyDown(OIS::KC_J)) _force_to_apply -= right * _force;
		if (input->isKeyDown(OIS::KC_L)) _force_to_apply += right * _force;

		// Forward/back
		if (input->isKeyDown(OIS::KC_K)) _force_to_apply += forward * _force;
		if (input->isKeyDown(OIS::KC_I)) _force_to_apply -= forward * _force;
