...
namespace {
    ...
    typedef Core::AI::AbstractProximityDatabase<AbstractVehicle*> ProximityDatabase;
    typedef Core::AI::AbstractTokenForProximityDatabase<AbstractVehicle*> ProximityToken;


    // ----------------------------------------------------------------------------


    class Boid : public Core::AI::CFpsVehicle
    {
    public:
        ...
        void reset (void)
        {
            ...
            regenerateOrthonormalBasisUF (Vec3Utilities::RandomUnitVector ());

            // randomize initial position
            setPosition (Vec3Utilities::RandomVectorInUnitRadiusSphere () * 20);
        ...
        void draw (void)
        {
			drawBasic3dSphericalVehicle (*this, Color::gGray70);
            // drawTrail ();
        }
        ...
        // basic flocking
        reVector3Df steerToFlock (void)
        {
            // avoid obstacles if needed
            // XXX this should probably be moved elsewhere
            const reVector3Df avoidance = steerToAvoidObstacles (1.0f, obstacles);
            if (avoidance != reVector3Df::ZERO) return avoidance;
            ...
            const reVector3Df separation = steerForSeparation (separationRadius,
                                                        separationAngle,
                                                        neighbors);
            const reVector3Df alignment  = steerForAlignment  (alignmentRadius,
                                                        alignmentAngle,
                                                        neighbors);
            const reVector3Df cohesion   = steerForCohesion   (cohesionRadius,
                                                        cohesionAngle,
                                                        neighbors);

            // apply weights to components (save in variables for annotation)
            const reVector3Df separationW = separation * separationWeight;
            const reVector3Df alignmentW = alignment * alignmentWeight;
            const reVector3Df cohesionW = cohesion * cohesionWeight;

            // annotation
            // const float s = 0.1;
            // annotationLine (position, position + (separationW * s), Color::gRed);
            // annotationLine (position, position + (alignmentW  * s), Color::gOrange);
            // annotationLine (position, position + (cohesionW   * s), Color::gYellow);
        ...
        void sphericalWrapAround (void)
        {
            ...
            if (position().length() > worldRadius)
            {
                ...
                setPosition (position().sphericalWrapAround (reVector3Df::ZERO,
                                                             worldRadius));
    ...
    // ---------------------------------------------- xxxcwr111704_terrain_following
        ...
        void regenerateLocalSpace (const reVector3Df& newVelocity,
        ...
        void regenerateLocalSpaceForTerrainFollowing  (const reVector3Df& newVelocity,
                                                       const float /* elapsedTime */)
        {

            // XXX this is special case code, these should be derived from arguments 
			reVector3Df tmp = position();
			tmp.normalise();
            const reVector3Df surfaceNormal = tmp;
            const reVector3Df surfacePoint = surfaceNormal * 40.0f; 
            // XXX this is special case code, these should be derived from arguments //

            const reVector3Df newUp = surfaceNormal;
            const reVector3Df newPos = surfacePoint;
            const reVector3Df newVel = newVelocity.perpendicularComponent(newUp);
            const float newSpeed = newVel.length();
            const reVector3Df newFor = newVel / newSpeed;
    ...
    public:
        ...
        void open (void)
        {
			...
			OpenSteerDemo::camera.povOffset.x = 0;
			OpenSteerDemo::camera.povOffset.y = 0.5;
			OpenSteerDemo::camera.povOffset.z = -2;
        ...
        void redraw (const float currentTime, const float elapsedTime)
        {
            ...
            OpenSteerDemo::drawCircleHighlightOnVehicle (nearMouse, 1, Color::gGray70);

            // highlight selected vehicle
            OpenSteerDemo::drawCircleHighlightOnVehicle (selected, 1, Color::gGray50);
            ...
            case 0:
                {
                    ...
                    const reVector3Df divisions (div, div, div);
        ...
        {void draw (const bool filled, const Color& color, const reVector3Df& vp) const
            {drawSphereObstacle (*this, 10.0f, filled, color, vp);}};

        class RO : public RectangleObstacle
        {void draw (const bool, const Color& color, const reVector3Df&) const
        ...
        void initObstacles (void)
        {
            ...
            const float m = -p;
            const float z = 0.0f;
            outsideSphere1.position().set (p, z, z);
            outsideSphere2.position().set (m, z, z);
            outsideSphere3.position().set (z, p, z);
            outsideSphere4.position().set (z, m, z);
            outsideSphere5.position().set (z, z, p);
            outsideSphere6.position().set (z, z, m);

            reVector3Df tiltF = reVector3Df (1.0f, 1.0f, 0.0f);
			tiltF.normalise();
            reVector3Df tiltS (0.0f, 0.0f, 1.0f);
            reVector3Df tiltU = reVector3Df (-1.0f, 1.0f, 0.0f);
			tiltU.normalise ();
        ...
        static void tempDrawRectangle (const RectangleObstacle& rect, const Color& color)
        {
            ...
            reVector3Df v1 = rect.globalizePosition (reVector3Df ( w,  h, 0));
            reVector3Df v2 = rect.globalizePosition (reVector3Df (-w,  h, 0));
            reVector3Df v3 = rect.globalizePosition (reVector3Df (-w, -h, 0));
            reVector3Df v4 = rect.globalizePosition (reVector3Df ( w, -h, 0));
        ...
        static void tempDrawBox (const BoxObstacle& box, const Color& color)
        {
            ...
            const reVector3Df p = box.position ();
            const reVector3Df s = box.side ();
            const reVector3Df u = box.up ();
            const reVector3Df f = box.forward ();

            const reVector3Df v1 = box.globalizePosition (reVector3Df ( w,  h,  d));
            const reVector3Df v2 = box.globalizePosition (reVector3Df (-w,  h,  d));
            const reVector3Df v3 = box.globalizePosition (reVector3Df (-w, -h,  d));
            const reVector3Df v4 = box.globalizePosition (reVector3Df ( w, -h,  d));

            const reVector3Df v5 = box.globalizePosition (reVector3Df ( w,  h, -d));
            const reVector3Df v6 = box.globalizePosition (reVector3Df (-w,  h, -d));
            const reVector3Df v7 = box.globalizePosition (reVector3Df (-w, -h, -d));
            const reVector3Df v8 = box.globalizePosition (reVector3Df ( w, -h, -d));
