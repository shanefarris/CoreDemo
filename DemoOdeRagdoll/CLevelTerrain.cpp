...
void CLevelTerrain::Update(f32 elapsedTime, OIS::Keyboard* Input, OIS::Mouse* Mouse)
{
	...
	if (ragdoll->isPhysical() && 
		!ragdoll->isStatic() && 
		ragdoll->isAwake() && 
		ragdoll->_timeSincePhysical > 10.0)
	{
		//radgoll->turnToStone();
		ragdoll->sleep();
	}
...
bool CLevelTerrain::Load()
{
	...
	SceneNode* node = m_SceneManager->getRootSceneNode()->createChildSceneNode("Solider_Node" + StringConverter::toString(i));
	node->setPosition(0.0,0.0,0);
	ragdoll = (OgreOde_Prefab::Ragdoll*)physics->LoadPrefabRagdoll(node, "solider.ogreode", "solider");
	ragdoll->releasePhysicalControl();
	...
	// Setup FrameListener
	m_FrameListenerFramework = new CFpsFrameListener(m_GameManager->GetRenderWindow(), m_GameManager->GetCurrCamera());
