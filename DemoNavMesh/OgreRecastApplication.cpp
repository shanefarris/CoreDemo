...
bool OgreRecastApplication::Load()
{
    CGameManager::Instance()->CreateScene("OctreeSceneManager", "main_scene");

     CGameManager::Instance()->GetSceneManager()->setAmbientLight(Ogre::ColourValue(0.5f, 0.5f, 0.5f));
    Ogre::Light* light =  CGameManager::Instance()->GetSceneManager()->createLight( "MainLight" );
    light->setPosition(20, 80, 50);
	CCameraManager::Instance()->GetCamera()->setPosition(-46.3106, 62.3307, 40.7579);
    CCameraManager::Instance()->GetCamera()->setOrientation(Ogre::Quaternion(0.903189, -0.247085, - 0.338587, - 0.092626));
    ...
    Ogre::Entity* mapE =  CGameManager::Instance()->GetSceneManager()->createEntity("Map", "dungeon.mesh");
    Ogre::SceneNode* mapNode =  CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->createChildSceneNode("MapNode");
    mapNode->attachObject(mapE);

    // Add gate
    Ogre::Entity *gateE = CGameManager::Instance()->GetSceneManager()->createEntity("Gate", "Gate.mesh");
    mGate =  CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->createChildSceneNode("GateNode");
    ...
    auto potE = CGameManager::Instance()->GetSceneManager()->createEntity("Pot1", "Pot.mesh");
    ...
    Ogre::Entity *pot2E =  CGameManager::Instance()->GetSceneManager()->createEntity("Pot2", "Pot.mesh");
    ...
    auto pot2ProxyE =  CGameManager::Instance()->GetSceneManager()->createEntity("Pot2Proxy", "Cylinder.mesh");
    ...
    mRecast = new OgreRecast( CGameManager::Instance()->GetSceneManager());    // Use default configuration
    if(SINGLE_NAVMESH) {
    ...
    // DetourTileCache navmesh creation
        ...
        InputGeom::drawConvexVolume(mGateHull,  CGameManager::Instance()->GetSceneManager());
    }
    ...
    if(!HUMAN_CHARACTERS)
		character->GetPlayerEntity()->setMaterialName("Cylinder/LightBlue");  // Give the first agent a different color
    ...
    mNavMeshNode = (Ogre::SceneNode*) CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->getChild("RecastSN");
    for (int i = 0; i < mNavMeshNode->numAttachedObjects(); i++) {
        Ogre::MovableObject *obj = mNavMeshNode->getAttachedObject(i);
		obj->setQueryFlags(Core::SQF_NAVMESH);
    }
    ...
    if (RAYCAST_SCENE || OgreRecast::STATIC_GEOM_DEBUG) // also when using static geometry because it's hard to ray query static geometry
        mapE->setQueryFlags(Core::SQF_NAVMESH);
	...
	Ogre::SceneNode *node = mCharacters[0]->GetPlayerNode();
    mChaseCam =  CGameManager::Instance()->GetSceneManager()->createCamera("AgentFollowCamera");
	...
	Ogre::Viewport *vp = CGameManager::Instance()->GetRenderWindow()->getViewport(0);
...
void OgreRecastApplication::Update(const f32& deltaTime)
{
    ...
    if(mApplicationState == STEER_AGENT) {
        ...
        if(!mMoveForwardKeyPressed && agentIsMoving) {
            ...
            firstAgent->GetPlayerNode()->yaw(Ogre::Degree(mMouseMoveX)); // Rotate character according to mouse move
    ...
    if(!SINGLE_NAVMESH && mDetourTileCache)
        mDetourTileCache->handleUpdate(deltaTime);

    // First update the agents using the crowd in which they are controlled
    if(mDetourCrowd)
        mDetourCrowd->updateTick(deltaTime);
    ...
    for(std::vector<Character*>::iterator iter=mCharacters.begin(); iter != mCharacters.end(); iter++) {
		...
		character->Update(deltaTime, CInputManager::Instance()->GetKeyboard(), CInputManager::Instance()->GetMouse());


        // Set new destinations depending on current demo mode

        // RANDOM WANDER BEHAVIOUR
        if( (mApplicationState == CROWD_WANDER || mApplicationState == STEER_AGENT) && character->getAgentID() != 0) // No random walking for first agent
        {
...
bool OgreRecastApplication::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
    ...
    return true; //BaseApplication::frameRenderingQueued(evt);
}

Ogre::Entity* OgreRecastApplication::createObstacle(Ogre::String name, Ogre::Vector3 position, Ogre::Vector3 scale) {
    Ogre::SceneNode *mapNode =  CGameManager::Instance()->GetSceneManager()->getSceneNode("MapNode");
    Ogre::SceneNode *node = mapNode->createChildSceneNode(name+"Node");
    Ogre::Entity *ent =  CGameManager::Instance()->GetSceneManager()->createEntity(name, "Pot.mesh");
...
bool OgreRecastApplication::mousePressed( const OIS::MouseEvent &arg, OIS::MouseButtonID id )
{
    ...
    if(queryCursorPosition(rayHitPoint, Core::SQF_NAVMESH)) {
    ...
    return true; //BaseApplication::mousePressed(arg, id);
}
...
bool OgreRecastApplication::keyPressed( const OIS::KeyEvent &arg )
{
    ...
    if(arg.key == OIS::KC_RETURN) {
        ...
        switch (mApplicationState) {
            case SIMPLE_PATHFIND:   // Simple -> wander
                ...
                mCrosshair->show();
                CGameManager::Instance()->GetRenderWindow()->getViewport(0)->setCamera(CCameraManager::Instance()->GetCamera());
                break;
            ...
            case FOLLOW_TARGET:     // Chase -> steer
                ...
                CGameManager::Instance()->GetRenderWindow()->getViewport(0)->setCamera(mChaseCam);  // Set chase camera on first agent
                break;
    ...
    if(!SINGLE_NAVMESH && mApplicationState != STEER_AGENT && arg.key == OIS::KC_DELETE) {
        ...
        if (queryCursorPosition(rayHitPoint, Core::SQF_OBSTACLE, false, &rayHitObject)) {
    ...
    if(arg.key == OIS::KC_K && !SINGLE_NAVMESH) {
        ...
        if (queryCursorPosition(rayHitPoint, DEFAULT_MASK, false)) {
            ...
            Ogre::Entity* palletE =   CGameManager::Instance()->GetSceneManager()->createEntity("WoodPallet.mesh");
            Ogre::SceneNode* palletN =   CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->createChildSceneNode();
        ...
        if (queryCursorPosition(rayHitPoint, DEFAULT_MASK, false, &rayHitObject)) {
            ...
            if( std::find(mWalkableObjects.begin(), mWalkableObjects.end(), rayHitObject) != mWalkableObjects.end() ) {
                ...
                rayHitObject->getParentSceneNode()->detachObject(rayHitObject);
                 CGameManager::Instance()->GetSceneManager()->destroyEntity((Ogre::Entity*)rayHitObject);
            }
        }
    }
...
bool OgreRecastApplication::keyReleased(const OIS::KeyEvent &arg)
{
    ...
    return true; //BaseApplication::keyReleased(arg);
}
...
Ogre::SceneNode* OgreRecastApplication::getOrCreateMarker(Ogre::String name, Ogre::String materialName)
{
    ...
    try {
        result = (Ogre::SceneNode*)( CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->getChild(name+"Node"));
    } catch(Ogre::Exception ex) {
        result =  CGameManager::Instance()->GetSceneManager()->getRootSceneNode()->createChildSceneNode(name+"Node");
        Ogre::Entity* ent =  CGameManager::Instance()->GetSceneManager()->createEntity(name, "Cylinder.mesh");
...
Character* OgreRecastApplication::createCharacter(Ogre::String name, Ogre::Vector3 position)
{
    ...
    Character *character = new Character(name,  CGameManager::Instance()->GetSceneManager(), mDetourCrowd, mDebugDraw, position);
...
bool OgreRecastApplication::queryCursorPosition(Ogre::Vector3 &rayHitPoint, unsigned long queryflags, bool clipToNavmesh, Ogre::MovableObject **rayHitObject)
{
    // Do ray scene query
    //send a raycast straight out from the camera at the center position
    Ogre::Ray mouseRay = CCameraManager::Instance()->GetCamera()->getCameraToViewportRay(0.5, 0.5);
...
// TODO: destroy queries using scenemgr::destroyRayQuery or reuse one query object by storing it in a member variable
    mRayScnQuery =  CGameManager::Instance()->GetSceneManager()->createRayQuery(Ogre::Ray(), queryMask);
...
INT WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPSTR strCmdLine, INT )
{
	...
	{
		Core::CGame game;
		game.AddGameState(new OgreRecastApplication());
		game.Run();
	} 
	catch(Exception& e) 
	{
